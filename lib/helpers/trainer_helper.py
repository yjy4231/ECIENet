...
class Trainer(object):
    def __init__(self,
                 ...
                 warmup_lr_scheduler,
                 logger):
        self.tb_writer = SummaryWriter(log_dir=os.path.join('log/add_att_lr08/', 'tensorboard'))
        self.num = 0
        ...
        for batch_idx, (inputs,calibs,coord_ranges, targets, info) in enumerate(self.train_loader):
            ...
            if loss_weights is not None:
                ...
                for key in loss_weights.keys():
                    ...
                    self.tb_writer.add_scalar(key, loss_terms[key], self.num)
            self.tb_writer.add_scalar('total_loss', total_loss, self.num)
            self.num += 1
            total_loss.backward()
            self.optimizer.step()
